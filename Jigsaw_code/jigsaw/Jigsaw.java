...
public class Jigsaw {
	...
	int value2 = 0;//放错位的数码与其正确位置的曼哈顿距离之和
	int value3 = 0;//放错位的数码与其正确位置的欧拉距离之和
	for (index = 1 ; index <= dimension * dimension ; ++index) {
	    if (index != nodeState[0]) {
		//  求解曼哈顿距离
		int cx, cy, dx, dy;
		...
		value2 += Math.abs(dx - cx) + Math.abs(dy - cy);
		//求解欧拉距离
		value3 += Math.sqrt((cx - dx) * (cx - dx) + (cy - dy) * (cy - dy));
	    }
	}
	int value =8 * value1 + 5 * value2 + 5 * value3;
